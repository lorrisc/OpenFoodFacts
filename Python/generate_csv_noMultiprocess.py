# Generation of all the csv corresponding to the database tables without multiprocessing
...
df_product = pd.read_csv("input/donnees_traite_small50000.csv", sep=";", dtype=dtypes)
outputPath = "output/"
...
def add_sales_data(row):
    global sales_data
    nutriscore_grade = row["nutriscore_grade"]
    ecoscore_grade = row["ecoscore_grade"]
    idproduct = row["id_product"]
    sales_data.extend(generate_sales(idproduct,nutriscore_grade,ecoscore_grade))
    
df_product.apply(add_sales_data, axis=1)
...
df_sales.to_csv(outputPath + "sales.csv", sep=";", index=False)
...
# PRODUCT
# df_product = df_product.reindex(columns=["id_product","idcountry"])
